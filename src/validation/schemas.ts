...
export const adFormatRulesSchema = z
  .object({
    ...
    aspectRatios: z
      .array(z.string().regex(/^[1-9]\d*:[1-9]\d*$/))
      .optional()
      .nullable(),
  ...
  .superRefine((v, ctx) => {
    if (v.minWidth != null && v.maxWidth != null && v.minWidth > v.maxWidth) {
      ctx.addIssue({
    ...
    if (v.minHeight != null && v.maxHeight != null && v.minHeight > v.maxHeight) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['minHeight'],
